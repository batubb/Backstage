...
app.post('/createCustomerAndSubscription', (request, response) => {
    ...
    if (typeof body.user === 'undefined' || typeof body.influencer === 'undefined' || typeof body.token === 'undefined' || typeof body.planId === 'undefined') {
    ...
    async function start() {
        try {
            ...
            for (let i = 0; i < followListKeys.length; i++) {
                ...
                const postsValue = await admin.database().ref('posts').child(element.uid).once('value');
                const userValue = await admin.database().ref('users').child(element.uid).once('value');
                var postsArray = [];
    ...
    async function start() {
        try {
            ...
            for (let i = 0; i < followListKeys.length; i++) {
                const k = followListKeys[i];
                const element = followList[k];
                const postsValue = await admin.database().ref('stories').child(element.uid).once('value');
                const userValue = await admin.database().ref('users').child(element.uid).once('value');
                var storiesArray = [];
                var myStoriesArray = [];
                ...
                postsValue.forEach(post => {
                    if (post.val()) {
                        if (new Date().getTime() - post.val().timestamp < 24 * 60 * 60 * 1000) {
                            storiesArray.push(post.val());
                        }
                    }
                });

                storiesArray.sort(function (a, b) { return a.timestamp - b.timestamp; });
            ...
            const postsValue = await admin.database().ref('stories').child(uid).once('value');
            var myStoriesArray = [];
            ...
            myStoriesArray.sort(function (a, b) { return a.timestamp - b.timestamp; });
            ...
            return response.status(200).send(JSON.stringify({ userStoriesArray, myStoriesArray }));
        } catch (error) {
            console.log(error);
            return response.status(400).send(JSON.stringify({ code: config.CODE_702, message: config.CODE_702_MESSAGE }));
        }
            ...
            if (liveValue.val()) {
                ...
                for (let i = 0; i < followList.length; i++) {
                    ...
                    for (let j = 0; j < liveValueArray.length; j++) {
                        ...
                        if (element.uid === data.user.uid && new Date().getTime() > data.timestamp + 5000) {
                            liveArray.push({
                                ...data,
                                active: element.active,
                                expired: element.expired,
...
app.get('/livestreamControl', (request, response) => {
    ...
    async function start() {
        ...
        return response.status(200).send(JSON.stringify({ code: 200, message: 'Done.' }));
    }
        ...
        for (let i = 0; i < liveValKeys.length; i++) {
            try {
                ...
                if (typeof element.liveId !== 'undefined') {
                    const livestreamResponse = await mux_instance.get('/video/v1/live-streams/' + element.liveId);
                    const status = livestreamResponse.data.data.status;

                    if (status !== 'active' && new Date().getTime() - element.timestamp > config.MUX_DELAY) {
                        await admin.database().ref('live').child(element.uid).set(null);
                        await admin.database().ref('posts').child(element.user.uid).child(element.uid).child('active').set(true);
                    }
                } else {
                    await admin.database().ref('live').child(element.uid).set(null);
                }

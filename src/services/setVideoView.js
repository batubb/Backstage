...
export default async function setVideoView(uid, video) {
    ...
    const videoViewData = await database().ref('views').child(video.uid).once('value');
    const viewCount = (Object.keys(videoViewData.val())).length;

    await database().ref('posts').child(video.user.uid).child(video.uid).child('view').set(viewCount);

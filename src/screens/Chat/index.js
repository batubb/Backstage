...
import { View, Dimensions, FlatList, TouchableOpacity, TextInput, Platform, SafeAreaView, RefreshControl, KeyboardAvoidingView } from 'react-native';
import { observer } from 'mobx-react';
import { Icon } from 'react-native-elements';
import { Loading, Header, Text, MyImage, Button } from '../../components';
import { constants } from '../../resources';
import { sendComment, setLikeCommentStatus, shareItem, getUserPosts } from '../../services';
...
class Chat extends Component {
    ...
    renderProfileTop = (user = this.state.user) => {
        return (
            <View style={{ width: width, flexDirection: 'row', padding: 15, alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Button
                    buttonStyle={{ width: (width - 200) / 2 - 5, backgroundColor: this.state.general ? '#FFF' : constants.BACKGROUND_COLOR, borderWidth: 1, borderColor: '#FFF', padding: 5, borderRadius: 16 }}
                    textStyle={{ color: this.state.general ? constants.BACKGROUND_COLOR : '#FFF', fontSize: 12 }}
                    text="General"
                    onPress={() => this.setState({ general: true })}
                />
                <Button
                    buttonStyle={{ width: (width - 200) / 2 - 5, backgroundColor: this.state.general ? constants.BACKGROUND_COLOR : '#FFF', borderWidth: 1, borderColor: '#FFF', padding: 5, borderRadius: 16 }}
                    textStyle={{ color: this.state.general ? '#FFF' : constants.BACKGROUND_COLOR, fontSize: 12 }}
                    text="Contents"
    ...
    commentBar = () => {
        ...
        return (
            ...
            <View style={{ alignItems: 'center', width: width, backgroundColor: constants.BACKGROUND_COLOR, paddingVertical: 10 }}>
                {
                    reply ?
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: width - 40, paddingVertical: 5 }}>
                            <Text
                            ...
                            <TouchableOpacity onPress={() => this.showReplyTab(null, false)}>
                                <View style={{ padding: 5 }}>
                                    <Icon name="close" color="gray" type="material-community" size={16} />
                                </View>
                            </TouchableOpacity>
                        </View>
                        :
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: width - 20, borderRadius: 4, backgroundColor: constants.BAR_COLOR }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <TextInput
                            ...
                            style={{ fontFamily: Platform.OS === 'ios' ? 'Avenir' : 'sans-serif-condensed', color: '#FFF', width: width - 110, fontSize: 16, padding: 10 }}
                            underlineColorAndroid="transparent"
                            onChangeText={commentInput => this.setState({ comment: commentInput })}
                            value={comment}
                        ...
                        comment !== '' ?
                            <TouchableOpacity onPress={() => this.sendComment()}>
                                <View style={{ padding: 10 }}>
                                    <Icon name="send" color="#FFF" type="material-community" size={16} />
                                </View>
                            </TouchableOpacity>
                            :
                            null
            ...
            <SafeAreaView style={{ width, alignItems: 'center', flex: 1 }}>
                ...
                >
                    <FlatList
                        ...
                        renderItem={({ item, index }) => <View style={{ paddingTop: 15, width: (width), alignItems: 'center', backgroundColor: constants.BAR_COLOR, marginTop: 10 }}>
                            ...
                            <View style={{ width: (width), flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <TouchableOpacity onPress={() => this.likeComment(item, index, item.likeStatus)}>
                                        <View style={{ padding: 10, flexDirection: 'row', alignItems: 'center' }}>
                                            <Icon name={item.likeStatus ? 'heart' : 'heart-outline'} color="#FFF" type="material-community" size={16} />
                                            {
                                                item.likeCount !== 0 ?
                                                    <Text
                                    ...
                                    <TouchableOpacity onPress={() => this.showReplyTab(item)}>
                                        <View style={{ padding: 10 }}>
                                            <Icon name="reply" color="#FFF" type="material-community" size={16} />
                                        </View>
                                    </TouchableOpacity>
                                </View>
                                {
                                    item.reply.length !== 0 ?
                                        <TouchableOpacity onPress={() => this.seeThread(item, index)}>
                                            <View style={{ padding: 10, flexDirection: 'row' }}>
                                                <Icon name={item.showReply ? 'chevron-up' : 'chevron-down'} color="#FFF" type="material-community" size={16} />
                                                <Text
                                                    text="See Thread"
                            ...
                            {item.showReply ? this.renderReply(item.reply.slice(0, 5), item, index) : null}
                        </View>}
                    />
                    {this.commentBar()}
                </KeyboardAvoidingView>
        ...
        return (
            <FlatList
                ...
                renderItem={({ item, index }) =>
                    <View style={{ width: (width), alignItems: 'flex-end', backgroundColor: constants.BACKGROUND_COLOR }}>
                        <View style={{ backgroundColor: constants.BAR_COLOR, marginTop: 10, paddingTop: 10, paddingHorizontal: 10, borderTopLeftRadius: 8, borderBottomLeftRadius: 8 }}>
                            <View style={{ width: (width) - 60, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row', width: (width) - 110, justifyContent: 'space-between' }}>
                            ...
                            <View style={{ width: (width) - 60, flexDirection: 'row', alignItems: 'center' }}>
                                <TouchableOpacity onPress={() => this.likeComment(item, index, item.likeStatus, true, mainComment, mainCommentIndex)}>
    ...
    renderPosts = () => {
        return (
            <FlatList
                ...
                keyExtractor={(item) => item.uid}
                numColumns={3}
                renderItem={({ item }) => <View style={{ width: (width / 3), alignItems: 'center', marginBottom: 5 }}>
                    <TouchableOpacity onPress={() => this.goTo('Comments', item)}>
                        <View style={{ borderRadius: 16, width: (width / 3) - 10, height: (1.5) * ((width / 3) - 10), backgroundColor: '#4d4d4d', alignItems: 'center', justifyContent: 'center' }}>
                            <MyImage
                                style={{ width: (width / 3) - 10, height: (1.5) * ((width / 3) - 10), borderRadius: 16 }}
                                photo={item.photo}
                            />
                            <LinearGradient
                                colors={['transparent', 'transparent', constants.BACKGROUND_COLOR]}
                                style={{ width: (width / 3) - 10, height: (1.5) * ((width / 3) - 10), borderRadius: 16, position: 'absolute' }}
                            />
                            <View style={{ position: 'absolute', bottom: 0, paddingVertical: 5, paddingHorizontal: 10, width: (width / 3) - 10, alignItems: 'flex-end' }}>
                                <Text
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <Text
                                        ...
                                        style={{ fontSize: 12, fontWeight: 'normal', marginRight: 5 }}
                                    />
                                    <Icon name="account-outline" color="#FFF" type="material-community" size={12} />
                                </View>
                            </View>
                        </View>
                    </TouchableOpacity>

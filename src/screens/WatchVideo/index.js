...
import { View, Dimensions, TouchableOpacity, Alert, TextInput, Platform, Keyboard, FlatList } from 'react-native';
...
import { checkSubscribtion, checkUserInfo, setVideoView, getVideoInfo, shareItem, report, sendComment } from '../../services';
...
class WatchVideo extends Component {
    ...
    deleteVideo = async () => {
        // TODO Video editleme
        await database().ref('posts').child(Store.user.uid).child(this.state.video.uid).set(null);
        this.setState({ optionsVisible: false });
        this.props.navigation.dispatch(StackActions.pop());
    }

    shareVideo = async (text = `Hey did you watch this video on BackStage. ${this.state.video.title} is live!`) => {
    ...
    goTo = (route, info = null) => {
        ...
        if (route === 'Comments') {
            const replaceActions = StackActions.push(route, { video: info, comments: this.state.comments });
    ...
    sendComment = async () => {
        await sendComment(Store.user, this.state.video, this.state.comment, this.state.reply);
                ...
                <View style={{ position: 'absolute', bottom: 0, width: width, alignItems: 'center', paddingVertical: BOTTOM_PADDING }}>
                    <View style={{ width: width - 40, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        <TouchableOpacity onPress={() => this.goTo('Comments', video)}>
                            <Text
                                text={`${followerCount(video.comments)} comments`}
                            />
                        </TouchableOpacity>
                        <Text
                            text={`${followerCount(video.view)} views`}
                        />
                    </View>
                    <View style={{ width: width - 40, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                        <TouchableOpacity onPress={() => this.setState({ paused: !paused })}>
                            <View style={{ width: 40 }}>
                                <Icon name={paused ? 'play' : 'pause'} color="#FFF" type="material-community" />
                            </View>
                        </TouchableOpacity>
                        <Slider
                            ...
                            step={2}
                            thumbStyle={{ height: 20, width: 20, backgroundColor: '#424242' }}
    ...
    renderLivePlayer = (video) => {
        return (
            <View style={{ flex: 1, position: 'absolute' }}>
                {
                    this.state.finished ?
                        <SafeAreaView style={{ flex: 1, width: width, height: height, alignItems: 'center', justifyContent: 'center' }}>
                            <Text
                                style={{ fontSize: 12 }}
                ...
                <SafeAreaView style={{ position: 'absolute', bottom: BOTTOM_PADDING, width, alignItems: 'center', height, justifyContent: 'flex-end' }}>
                    {
                        this.state.keyboard ?
                            <TouchableOpacity style={{ position: 'absolute', bottom: 0 }} onPress={() => this.textinput.blur()}>
                                <View style={{ backgroundColor: constants.BACKGROUND_COLOR, width, alignItems: 'center', height, opacity: 0.4 }} />
                            </TouchableOpacity>
                            :
                            null
                    }
    ...
    commentBar = () => {
        ...
        return (
            <KeyboardAvoidingView behavior="padding" style={{ alignItems: 'center', width: width }}>
                <FlatList
                    data={comments}
                    keyExtractor={(item) => item.uid}
                    renderItem={({ item, index }) => <View style={{ width: (width - 20), alignItems: 'flex-start', marginVertical: 2.5 }}>
                        <View style={{ backgroundColor: constants.BAR_COLOR, opacity: this.state.keyboard ? 1 : 0.6, borderRadius: 24, padding: 10 }}>
                            <Text
                                text={item.user.name}
                                style={{ fontSize: 12 }}
                            />
                ...
                />
                <View style={{ width: (width - 20), flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: this.state.keyboard ? width - 20 : width - 70, borderRadius: 24, backgroundColor: constants.BAR_COLOR, marginTop: 2.5, opacity: this.state.keyboard ? 1 : 0.6 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <TextInput
                                ref={input => (this.textinput = input)}
                                placeholder="Write your comment..."
                                style={{ fontFamily: Platform.OS === 'ios' ? 'Avenir' : 'sans-serif-condensed', color: '#FFF', width: width - 70, fontSize: 12, padding: 15 }}
                    ...
                    {
                        !this.state.keyboard ?
                            <TouchableOpacity onPress={() => this.shareVideo(`Hey are you watching this live video? Let's watch ${this.state.influencer.username} together.`)}>
                                <View style={{ width: 45, height: 45, backgroundColor: '#FFF', borderRadius: 22.5, alignItems: 'center', justifyContent: 'center' }}>
                                    <Icon name="export-variant" color="#000" type="material-community" />
                                </View>
                            </TouchableOpacity>
                            :
                            null
            ...
            <View style={{ flex: 1, backgroundColor: constants.BACKGROUND_COLOR }}>
                ...
                {video.type === 'live' ? this.renderLivePlayer(video) : null}
                {
                    (loading || videoLoading) ?
                        <Loading
                            loadingStyle={{ position: 'absolute', width: width, height: height, alignItems: 'center', justifyContent: 'center', backgroundColor: '#000', opacity: 0.6 }}

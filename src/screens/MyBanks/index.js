...
import { View, Dimensions, TextInput, Platform, Alert, ScrollView, RefreshControl } from 'react-native';
...
class MyBanks extends Component {
    ...
    componentDidMount = async () => {
        this.setState({ loading: false, account: typeof Store.user.bankAccount === 'undefined' ? '' : Store.user.bankAccount });
    }

    saveInfo = async () => {
        if (this.state.account === '') {
            return Alert.alert('Oops', 'You have to enter your bank account.', [{ text: 'Okay' }]);
        }
                    ...
                    loading ?
                        ...
                        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} tintColor="white" />}>
                            ...
                            <Text
                                ...
                                style={{ fontSize: 12, fontWeight: 'normal', color: 'lightgray', marginHorizontal: 15 }}
                            />
                        </ScrollView>
                }
            </View>

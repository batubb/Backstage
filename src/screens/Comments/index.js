...
import { View, Dimensions, FlatList, TouchableOpacity, RefreshControl, KeyboardAvoidingView, SafeAreaView, Platform, TextInput, Alert } from 'react-native';
import { observer } from 'mobx-react';
import { StackActions } from '@react-navigation/native';
import { Icon } from 'react-native-elements';
import { Loading, Header, Text, MyImage } from '../../components';
import { constants } from '../../resources';
import { sendComment, setVideoComment, setLikeCommentStatus, checkSubscribtion } from '../../services';
...
class Comments extends Component {
    ...
    sendComment = async () => {
        ...
        if (this.state.anonymus) {
            await sendComment({ ...Store.user, type: 'anonymus', name: this.state.anonymus.nickname, photo: constants.DEFAULT_PHOTO }, this.state.video, this.state.comment, this.state.reply);
        } else {
    ...
    likeComment = async (item, index, status = false, reply = false, mainComment = null, mainCommentIndex = 0) => {
        ...
        await setLikeCommentStatus(Store.user, this.state.video, item, status, reply, mainComment);
    }

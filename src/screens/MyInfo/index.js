...
import { View, Dimensions, Platform, Alert, TextInput, ScrollView, RefreshControl } from 'react-native';
...
class MyInfo extends Component {
    ...
    componentDidMount = async () => {
        if (this.state.type === 'name') {
            this.setState({ input: typeof Store.user.name === 'undefined' ? '' : Store.user.name });
        } else if (this.state.type === 'biography') {
            this.setState({ input: typeof Store.user.biography === 'undefined' ? '' : Store.user.biography });
        } else if (this.state.type === 'username') {
            this.setState({ input: typeof Store.user.username === 'undefined' ? '' : Store.user.username });
        }
    ...
    saveInfo = async () => {
        ...
        if (this.state.type === 'username') {
            ...
            if (checkValid && Store.user.username !== this.state.input) {
                return Alert.alert('Oops', 'This username is taken. You have to choose another username.', [{ text: 'Okay' }]);
            }
        }
                    ...
                    loading ?
                        ...
                        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} tintColor="white" />}>
                            <TextInput
                                ...
                                style={{ fontFamily: Platform.OS === 'ios' ? 'Avenir' : 'sans-serif-condensed', margin: 10, width: width - 20, padding: 13, color: 'white', fontWeight: 'bold', fontSize: 14, borderRadius: 4, backgroundColor: '#424242' }}
